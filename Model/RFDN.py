...
def get_RFDN(in_nc=3, out_nc=3, upscale=4, checkpoint=None):

    model = RFDN(upscale=upscale, in_nc=in_nc, out_nc=out_nc)
    if checkpoint is not None:
        checkpoint = './Checkpoints/RFDN/' + checkpoint
...
if __name__ == '__main__':
    ...
    input = torch.randn(1, 3, 320, 180)  # 60.898 G
    # input = torch.randn(1, 3, 100, 100) # 3.952 G
    # input = torch.randn(1, 3, 200, 200)  # 16.520 G
    # input = torch.randn(1, 3, 1280, 640)
    flops, params = profile(model, inputs=(input,))

    print('flops :%.3f' % (flops / 1000 ** 3), 'G')  # 打印计算量
    print('params:%.3f' % (params / 1000 ** 2), 'M')  # 打印参数量
    print('params:%.3f' % (params / 1000 ** 1), 'K')  # 打印参数量