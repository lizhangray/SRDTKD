from Model.LBNet import common
import torch
import torch.nn as nn
from Model.LBNet.util.tools import extract_image_patches, reduce_mean, reduce_sum, same_padding, reverse_patches
from Model.LBNet.util.rlutrans_tiny import  Mlp, TransBlock
...
class LBNet(nn.Module):
    # def __init__(self, args, conv=common.default_conv, norm_layer=nn.LayerNorm):
    #     super(LBNet, self).__init__()
    def __init__(self, scale=4, n_feats=32, num_heads=8, rgb_range=255, n_colors=3,
                 conv=common.default_conv, norm_layer=nn.LayerNorm):
        super(LBNet, self).__init__()
...
def get_LBNet_tiny(checkpoint=None):

    model = LBNet(n_feats=18, num_heads=6)

    # param_key_g = 'params'
    if checkpoint is not None:
        # print('load pretrained model: {}!'.format(checkpoint))
        checkpoint = './Checkpoints/LBNet_tiny/' + checkpoint
        ...
        # model.load_state_dict({k.replace('module.', ''): v for k, v in ckpt['model_state_dict'].items()}, strict=True)
