...
def get_SwinIR(checkpoint=None):
    model = SwinIR(upscale=4, in_chans=3, img_size=64, window_size=8,
                img_range=1., depths=[6, 6, 6, 6], embed_dim=60, num_heads=[6, 6, 6, 6],
                mlp_ratio=2, upsampler='pixelshuffledirect', resi_connection='1conv')
    # model = SwinIR(upscale=4, in_chans=3, img_size=64, window_size=8,
    #                img_range=1., depths=[6], embed_dim=60, num_heads=[6, 6],
    #                mlp_ratio=2, upsampler='pixelshuffledirect', resi_connection='1conv')

    param_key_g = 'params'
    if checkpoint is not None:
        checkpoint = './Checkpoints/SwinIR/' + checkpoint
        ...
        model.load_state_dict(
            pretrained_model[param_key_g] if param_key_g in pretrained_model.keys() else pretrained_model, strict=False)

    return model


def get_SwinIR_GAN(checkpoint=None):
    model = SwinIR(upscale=4, in_chans=3, img_size=64, window_size=8,
...
if __name__ == '__main__':
    ...
    input = torch.randn(1, 3, 1280//4, 720//4)
